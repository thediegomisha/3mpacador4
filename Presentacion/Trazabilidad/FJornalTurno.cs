    ...
    public partial class FJornalTurno : Form
    {
        ...
        private void Cargar_turno()
        {
            ...
            try
            {
                ...
                {
                    while (reader.Read()) cbturno.Items.Add(reader["idturno"] + " - " + reader["nombre"]);
                }
        ...
        private void Lista_Grupo_turno(string ls_fecha_produccion)
        {
            dgvgrupo_turno_cab.Rows.Clear();
            var Lista = LGrupo_turno.Lista_grupo_turno(ls_fecha_produccion);
            foreach (var f in Lista)
                dgvgrupo_turno_cab.Rows.Add(null, null, f.idgrupo, f.descripcion, f.idusuario, f.nom_usuario, f.idturno,
                    f.nom_turno,
                    f.fecha_produccion, f.fecha_inicio, f.fecha_fin, f.flag_tercero == "1" ? true : false,
                    f.flag_estado == "1" ? true : false);
        }

        private void Lista_Grupo_turno_det(int li_idgrupo)
        {
            dgvlista_trab.Rows.Clear();
            var Lista = LGrupo_turno.Lista_grupo_turno_detalle(li_idgrupo);
            foreach (var f in Lista) dgvlista_trab.Rows.Add(f.idgrupo, f.trabajador, null, f.ult_cantidad, null);
        }
        ...
        private void btncrearturno_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show(@"Ingrese La Descripcion del Grupo", @"Aviso", MessageBoxButtons.OK,
                ...
                aux.idturno =
                    Convert.ToInt32(cbturno.Text.Substring(0, cbturno.Text.Contains("-").ToString().Length - 2));
            ...
            {
                MessageBox.Show(@"COLABORADOR NO REGISTRADO. " + ex.Message, @"ERROR", MessageBoxButtons.OK,
        ...
        {
            Cargar_turno();
            lblusuario.Text = Login.usuarioId1 + " " + Login.nombre1 + " " + Login.apaterno1;
        }
        ...
        public bool Existe_dni(string rol, DataGridView dgv)
        {
            ...
            {
                var verifica = f.Cells[0].Value.ToString();
                if (rol == verifica)
                {
        ...
        private void btnasigna_trab_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show(@"No Hay Ningun Trabajador para Asignar al Grupo", @"Aviso", MessageBoxButtons.OK,
                        MessageBoxIcon.Exclamation);
                    return;
                }
                ...
                MessageBox.Show(@"TRABAJADORES ASIGNADOS AL GRUPO SATISFACTORIAMENTE.", @"Mensaje", MessageBoxButtons.OK,
                    MessageBoxIcon.Information);
                Lista_Grupo_turno_det(Convert.ToInt32(lblidgrupo.Text));
                ConexionGral.desconectar();
            }
            catch (MySqlException ex)
            {
                MessageBox.Show(@"TRABAJADOR NO REGISTRADO. " + ex.Message, @"ERROR", MessageBoxButtons.OK,
                    MessageBoxIcon.Error);
                throw;
            }
        }
        ...
        private void dgvlista_trab_CellPainting(object sender, DataGridViewCellPaintingEventArgs e)
        {
            if (e.ColumnIndex == 2 && e.RowIndex > -1)
            {
                ...
                var img = Resources.qr;
                e.Graphics.DrawImage(img, e.CellBounds.Left + 30, e.CellBounds.Top + 3, 18,
                    15); // centrar la imagen en el boton
                e.Handled = true;
            }
        }
        ...
        private void dgvgrupo_turno_cab_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (dgvgrupo_turno_cab.Columns[e.ColumnIndex].Index == 0) //EDITAR
            {
                ...
                cbturno.Text = dgvgrupo_turno_cab.CurrentRow.Cells[6].Value + " - " +
                               dgvgrupo_turno_cab.CurrentRow.Cells[7].Value;
                ...
                cbflag_tercero.Checked =
                    dgvgrupo_turno_cab.CurrentRow.Cells[11].Value.ToString() == "True" ? true : false;
                lblestado.Text = dgvgrupo_turno_cab.CurrentRow.Cells[12].Value.ToString() == "True"
                    ? "ACTIVO"
                    : "ANULADO";
            }
            else if (dgvgrupo_turno_cab.Columns[e.ColumnIndex].Index == 1) // ELIMINAR
            {
                var rpta = MessageBox.Show(
                    @"¿ ESTA SEGURO DE ELIMINAR EL GRUPO " + dgvgrupo_turno_cab.CurrentRow.Cells[3].Value + " ?"
                ...
                if (rpta == DialogResult.Yes)
                {
                    ...
                    try
                    {
                        ...
                        comando.Parameters.AddWithValue("p_id",
                            Convert.ToInt32(dgvgrupo_turno_cab.CurrentRow.Cells[2].Value));
                        comando.ExecuteNonQuery();
                        MessageBox.Show(@"GRUPO TURNO SE ELIMINO SATISFACTORIAMENTE.", @"Mensaje", MessageBoxButtons.OK,
                            MessageBoxIcon.Information);
                        ConexionGral.desconectar();
                    }
                    catch (Exception ex)
        ...
        private void btnactualizar_turno_Click(object sender, EventArgs e)
        {
            if (editar)
            {
                try
                {
                    ...
                    if (cbturno.SelectedIndex == 0)
                    {
                        MessageBox.Show(@"Seleccione un Turno", @"Aviso", MessageBoxButtons.OK,
                            MessageBoxIcon.Exclamation);
                        return;
                    }
                    ...
                    comando.ExecuteNonQuery();
                    MessageBox.Show(@"GRUPO TURNO ACTUALIZADO SATISFACTORIAMENTE.", @"Mensaje", MessageBoxButtons.OK,
                        MessageBoxIcon.Information);
                    Lista_Grupo_turno(aux.fecha_produccion.ToString("yyyy-MM-dd"));
                    ConexionGral.desconectar();
                    return;
...
" + ex.Message, @"ERROR", MessageBoxButtons.OK,
        ...
        private void dgvgrupo_turno_cab_SelectionChanged(object sender, EventArgs e)
        {
            if (dgvgrupo_turno_cab.SelectedRows.Count > 0)
            {
                ...
                lbldesc_grupo.Text = ls_grupo_desc;
        ...
        private void btnbuscar_trab_Click(object sender, EventArgs e)
        {
            if (dgvgrupo_turno_cab.SelectedRows.Count <= 0)
            {
                MessageBox.Show(@"Debe Selecccionar un Grupo", @"Aviso...!!!", MessageBoxButtons.OK,
                    MessageBoxIcon.Exclamation);
                return;
            }
            ...
            if (FTraslado_trabajador.estado_transferencia)
            {
                if (dgvlista_trab.RowCount > 0)
                {
                    foreach (var y in FTraslado_trabajador.lista_datos)
                        if (Existe_dni(y.dni, dgvlista_trab) == false)
                            dgvlista_trab.Rows.Add(y.dni, y.trabajador);
                    lblnro_trab.Text = dgvlista_trab.RowCount.ToString();
                }
                else
                {
                    foreach (var y in FTraslado_trabajador.lista_datos) dgvlista_trab.Rows.Add(y.dni, y.trabajador);
                    lblnro_trab.Text = dgvlista_trab.RowCount.ToString();
                }
            }
        }
