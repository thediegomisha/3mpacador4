...
namespace _3mpacador4.Logica
{
    public class LConteo_manual
    {
        ...
        public static int Existe_Conteo_manual_x_fecha(string ls_fecha_produccion, int li_idpriceso)
        {
            int li_rpta = 0;
            ...
            string sql = @"select count(*) from tblconteo_jabas cj 
                            ...
                            pp.idpresentacion = (select x.idpresentacion from tblprograma_proceso x where x.idproceso = @idproceso)";
            var cmd = new MySqlCommand(sql, ConexionGral.conexion);
            cmd.Parameters.AddWithValue("@fecha_produccion", ls_fecha_produccion);
            cmd.Parameters.AddWithValue("@idproceso", li_idpriceso);
        ...
        public static int Conteo_Manual(int idproceso, int calibre, string fecha_produccion, int cantidad)
        {
            try
            {
                int rpta = 0;
